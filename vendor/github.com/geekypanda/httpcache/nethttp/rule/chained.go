...
// Claim validator
func (c *chainedRule) Claim(r *http.Request) bool {
	if !c.Rule.Claim(r) {
		return false
	}
	return c.next.Claim(r)
}

// Valid validator
func (c *chainedRule) Valid(w http.ResponseWriter, r *http.Request) bool {
	if !c.Rule.Valid(w, r) {
		return false
	}
	return c.next.Valid(w, r)
}
