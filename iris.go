...
func defaultServerOptions() ServerOptions {
	return ServerOptions{Cache: true, CacheMaxItems: 0, CacheResetDuration: 5 * time.Minute}
}
...
func New(options ...ServerOptions) *Server {
	...
	if options != nil && len(options) > 0 {
		...
		if options[0].Cache == false {
			...
			_server.router = NewMemoryRouter(options[0].CacheMaxItems, options[0].CacheResetDuration)
		}
	} else { // no options has passed to the function, default the options and set MemoryRouter
		opt := defaultServerOptions()
		_server.router = NewMemoryRouter(opt.CacheMaxItems, opt.CacheResetDuration) //the default will be the memory router
	}
