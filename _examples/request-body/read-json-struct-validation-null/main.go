...
type User struct {
	FirstName      sql.NullString `json:"-" validate:"required"`
...
func (u *User) Decode(data []byte) error {
	...
	err := json.Unmarshal(data, &fName)
	if err != nil {
		return fmt.Errorf("fname unmarshal %w", err)
	}

	u.FirstName = sql.NullString{String: fName.FirstName, Valid: true}
	err = json.Unmarshal(data, u)
	if err != nil {
		return fmt.Errorf("user unmarshal %w", err)
	}
